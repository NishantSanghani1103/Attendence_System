...
namespace CodeHtml.Staff
{
    public partial class Password : System.Web.UI.Page
    {
        ...
        {
            GetCon();
            da = new SqlDataAdapter("select Pass from StaffMst where Uname='" + Session["username"] + "'", con);
            ds = new DataSet();
            da.Fill(ds);
            return ds;
        }
        protected void btnchangepass_Click(object sender, EventArgs e)
        {
            ...
            if (user.Tables[0].Rows.Count > 0)
            {
                ...
                if (CurrentPass == dbpass)
                {
                    if (NewPass == ConPass)
                    {
                        cmd = new SqlCommand("update StaffMst set Pass='" + txtcpass.Text + "' where Uname='" + Session["username"] + "'", con);
                        cmd.ExecuteNonQuery();
